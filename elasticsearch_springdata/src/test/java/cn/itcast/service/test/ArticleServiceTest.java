...
public class ArticleServiceTest {
	...
	public void saveBatch(){
		...
		for (int i = 0; i < 100; i++) {
			...
			article.setContent(i+"DATAES-171 - 添加失效查询关键字支持 DATAES-194 - 测试可以清理  data 目录 DATAES-179 - 支持  Attachment 字段类型 DATAES-94 - 更新到最新版本的 elasticsearch 1.7.3 驱动器");
			articleService.save(article);
		}
	}
	...
	public void findpage(){
		Sort sort = new Sort(new Sort.Order(Sort.DEFAULT_DIRECTION.DESC,"id"));
	...
	public void findById(){
		Article article = new Article();
		article.setId(1001);
		System.out.println(articleService.findById(article));
	}
	
	@Test
	public void findConditionpage(){
		
		Pageable pageable = new PageRequest(0, 20);
		List<Article> findByContent = articleService.findByContent("添加",pageable);
